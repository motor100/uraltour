...
if (saveDataForm) {
  ...
  const programToEditorJs = document.getElementById('program-to-editorjs');
  ...
  // Initial Editor JS
  const program_editor = new EditorJS({

    holder: 'program-editorjs',
    ...
    data: (programToEditorJs && programToEditorJs.innerText) ? JSON.parse(programToEditorJs.innerText) : ''
  });

  // Отправка формы с перезагрузкой
  const saveDataInput = document.getElementById('save-data-input');
  const saveDataProgramInput = document.getElementById('save-data-program-input');
  ...
  saveDataForm.onsubmit = function(e) {
    ...
    .then((savedData) => {
      // Присваиваю saveDataInput.value значение savedData в виде строки
      saveDataProgramInput.value = JSON.stringify(savedData);
